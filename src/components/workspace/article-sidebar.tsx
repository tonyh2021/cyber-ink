    ...
    <>
      <aside
        className={`workspace-panel-left fixed left-0 top-0 h-full bg-surface-root flex flex-col z-40 transition-[width,padding] duration-300 ease-in-out overflow-hidden ${
          collapsed
            ? "w-0 md:w-14 items-center py-5 px-0"
            : "w-[220px] py-5 px-3"
        }`}
      >
        {/* Logo row */}
        ...
        <div className={`h-6 shrink-0 ${collapsed ? "hidden md:block" : ""}`} />

        {/* Nav links */}
        <nav
          className={`flex flex-col shrink-0 ${collapsed ? "hidden md:flex items-center gap-2" : "gap-0.5"}`}
        >
          {navItems.map((item) => {
        ...
        {/* Bottom section */}
        <div
          className={`flex flex-col shrink-0 ${collapsed ? "hidden md:flex items-center gap-3" : "gap-3"}`}
        >
          {collapsed ? (
            <button
